...
func resolveNameRun(cmd *cobra.Command, args []string, opts *resolveNameOpts) error {
	...
	ctx := context.Background()
	body := openapi.InlineObject5{
		ResourceName: openapi.PtrString(opts.resourceName),
	}
	result, response, err := client.AppliancesApi.AppliancesIdTestResolverNamePost(ctx, opts.applianceID).InlineObject5(body).Authorization(token).Execute()
	if err != nil {
